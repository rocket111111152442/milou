...
export async function GET(req: NextRequest) {
  try {
    ...
    const [listingsSnap, fromTx, toTx, clientMissions, providerMissions, clientDone, providerDone] =
      ...
      db.collection('missions').where('clientId', '==', uid).where('status', 'in', ['in_progress', 'disputed']).get(),
      db.collection('missions').where('providerId', '==', uid).where('status', 'in', ['in_progress', 'disputed']).get(),
